...
const ELEMENT_COLORS := {
	"fire": Color(0.85, 0.2, 0.15),   # Red
	"water": Color(0.2, 0.4, 0.85),   # Blue
	"earth": Color(0.2, 0.55, 0.2),   # Green
	"air": Color(0.85, 0.75, 0.1)     # Yellow
}
...
func _create_ui() -> void:
	...
	# Preview panel centered on screen
	_create_preview_panel(container)

	# List panel to the right of preview
	_create_list_panel(container)


func _create_preview_panel(parent: Control) -> void:
	var panel_container := Control.new()
	panel_container.custom_minimum_size = Vector2(PREVIEW_PANEL_WIDTH, PANEL_HEIGHT)
	panel_container.size = Vector2(PREVIEW_PANEL_WIDTH, PANEL_HEIGHT)
	panel_container.clip_contents = true  # Clip any overflow
	# Center horizontally and vertically
	panel_container.set_anchors_preset(Control.PRESET_CENTER)
	panel_container.position = Vector2(-PREVIEW_PANEL_WIDTH / 2.0, -PANEL_HEIGHT / 2.0)
	...
	_count_label.add_theme_color_override("font_color", Color(0.3, 0.3, 0.3))
	...
	# Preview box with sprite (no background)
	...
	# Hidden rect (kept for compatibility but invisible)
	_preview_rect = ColorRect.new()
	_preview_rect.set_anchors_preset(Control.PRESET_FULL_RECT)
	_preview_rect.color = Color(0, 0, 0, 0)  # Transparent
	_preview_rect.visible = false
	preview_container.add_child(_preview_rect)

	_preview_sprite = Label.new()
	_preview_sprite.text = "?"
	_preview_sprite.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_preview_sprite.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	_preview_sprite.set_anchors_preset(Control.PRESET_FULL_RECT)
	_preview_sprite.add_theme_font_size_override("font_size", 36)
	_preview_sprite.add_theme_color_override("font_color", Color(0.5, 0.5, 0.5))
	preview_container.add_child(_preview_sprite)

	# Actual creature sprite texture
	...
	_preview_name.text = "???"
	_preview_name.add_theme_font_size_override("font_size", 10)
	_preview_name.add_theme_constant_override("outline_size", 5)
	_preview_name.add_theme_color_override("font_outline_color", Color.BLACK)
	content_vbox.add_child(_preview_name)

	# Element label
	_preview_element = Label.new()
	_preview_element.text = "Unknown"
	_preview_element.add_theme_font_size_override("font_size", 8)
	_preview_element.add_theme_color_override("font_color", Color.BLACK)
	_preview_element.add_theme_constant_override("outline_size", 3)
	_preview_element.add_theme_color_override("font_outline_color", Color.WHITE)
	content_vbox.add_child(_preview_element)

	# Physical (height/weight) label
	_preview_physical = Label.new()
	_preview_physical.text = "?.?m  ?.?kg"
	_preview_physical.add_theme_font_size_override("font_size", 8)
	_preview_physical.add_theme_color_override("font_color", Color.BLACK)
	content_vbox.add_child(_preview_physical)

	# Stats label
	_preview_stats = Label.new()
	_preview_stats.text = "HP:?? ATK:?? DEF:?? SPD:??"
	_preview_stats.add_theme_font_size_override("font_size", 7)
	_preview_stats.add_theme_color_override("font_color", Color.BLACK)
	content_vbox.add_child(_preview_stats)

	# Separator line between stats and description
	...
	_preview_description.text = "This creature has not been discovered yet."
	_preview_description.add_theme_font_size_override("font_size", 7)
	_preview_description.add_theme_color_override("font_color", Color.BLACK)
	...
	# Close button at bottom of preview panel
...
func _create_list_panel(parent: Control) -> void:
	var panel_container := Control.new()
	panel_container.custom_minimum_size = Vector2(LIST_PANEL_WIDTH, PANEL_HEIGHT)
	panel_container.size = Vector2(LIST_PANEL_WIDTH, PANEL_HEIGHT)
	# Position to the right of centered preview panel
	panel_container.set_anchors_preset(Control.PRESET_CENTER)
	panel_container.position = Vector2(PREVIEW_PANEL_WIDTH / 2.0 + PANEL_GAP, -PANEL_HEIGHT / 2.0)
	parent.add_child(panel_container)

	var list_panel := NinePatchRect.new()
	list_panel.texture = preload("res://assets/sprites/ui/panels/panel_frame_48x40.png")
	...
	_scroll_container.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_SHOW_NEVER  # Hide scrollbar
...
func _create_list_entry(slot: int) -> Control:
	...
	container.custom_minimum_size = Vector2(120, 12)
	...
	var species: CreatureSpecies = _species_by_slot.get(slot, null)

	# Number label
	var num_label := Label.new()
	num_label.text = num_str
	num_label.add_theme_font_size_override("font_size", 7)
	...
	if species != null and GameState.is_species_discovered(species.species_id):
		num_label.add_theme_color_override("font_color", Color.BLACK)
		name_label.text = species.get_display_name()
		var element_color: Color = ELEMENT_COLORS.get(species.element, Color.BLACK)
		name_label.add_theme_color_override("font_color", element_color)
		name_label.add_theme_color_override("font_outline_color", Color.BLACK)
		name_label.add_theme_constant_override("outline_size", 4)
		container.gui_input.connect(_on_list_entry_input.bind(slot, species, true))
	else:
		num_label.add_theme_color_override("font_color", Color(0.5, 0.5, 0.5))
		name_label.text = "???"
		name_label.add_theme_color_override("font_color", Color(0.5, 0.5, 0.5))
		if species:
			container.gui_input.connect(_on_list_entry_input.bind(slot, species, false))
		else:
			container.gui_input.connect(_on_empty_slot_input.bind(slot))

	return container


func _on_list_entry_input(event: InputEvent, slot: int, species: CreatureSpecies, is_discovered: bool) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		_show_species_preview(slot, species, is_discovered)


func _on_empty_slot_input(event: InputEvent, slot: int) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
...
func _select_first_discovered() -> void:
	for slot in range(1, TOTAL_SLOTS + 1):
		var species: CreatureSpecies = _species_by_slot.get(slot, null)
		if species and GameState.is_species_discovered(species.species_id):
...
func _show_species_preview(slot: int, species: CreatureSpecies, is_discovered: bool) -> void:
	_selected_index = slot

	if is_discovered:
		_preview_name.text = species.get_display_name()
		_preview_element.text = species.element.capitalize()
		_preview_physical.text = "%.1fm  %.1fkg" % [species.height_m, species.weight_kg]
		...
		# Name and element in element color with black outline
		var element_color: Color = ELEMENT_COLORS.get(species.element, Color.BLACK)
		_preview_name.add_theme_color_override("font_color", element_color)
		_preview_name.add_theme_color_override("font_outline_color", Color.BLACK)
		_preview_element.add_theme_color_override("font_color", element_color)
		_preview_element.add_theme_color_override("font_outline_color", Color.BLACK)
		_preview_physical.add_theme_color_override("font_color", Color.BLACK)
		_preview_stats.add_theme_color_override("font_color", Color.BLACK)
		_preview_description.add_theme_color_override("font_color", Color.BLACK)

		# Show actual sprite if available, otherwise placeholder
		if species.sprite_texture:
			_preview_texture.texture = species.sprite_texture
			# Apply species sprite_scale
			var sprite_scale: float = species.sprite_scale if species.sprite_scale > 0 else 1.0
			_preview_texture.pivot_offset = _preview_texture.size / 2.0
			_preview_texture.scale = Vector2(sprite_scale, sprite_scale)
			...
			_preview_sprite.text = ":)"
			_preview_sprite.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
	else:
		_preview_name.text = "???"
		_preview_element.text = "Unknown"
		_preview_physical.text = "?.?m  ?.?kg"
		_preview_element.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
		_preview_element.add_theme_color_override("font_outline_color", Color.WHITE)
		_preview_name.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
		_preview_physical.add_theme_color_override("font_color", Color.BLACK)
		_preview_stats.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
		_preview_description.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
		_preview_stats.text = "HP:?? ATK:?? DEF:?? SPD:??"
		_preview_description.text = "This creature has not been discovered yet."

		# Show silhouette for undiscovered creatures that have a sprite
		if species.sprite_texture:
			_preview_texture.texture = species.sprite_texture
			# Apply species sprite_scale
			var sprite_scale: float = species.sprite_scale if species.sprite_scale > 0 else 1.0
			_preview_texture.pivot_offset = _preview_texture.size / 2.0
			_preview_texture.scale = Vector2(sprite_scale, sprite_scale)
			_preview_texture.modulate = Color(0.15, 0.15, 0.2)  # Dark silhouette
			_preview_texture.visible = true
			_preview_sprite.visible = false
		else:
			_preview_texture.visible = false
			_preview_sprite.visible = true
			_preview_sprite.text = "?"
			_preview_sprite.add_theme_color_override("font_color", Color(0.4, 0.4, 0.5))
...
func _show_empty_preview(slot: int) -> void:
	...
	_preview_element.text = "Unknown"
	_preview_physical.text = "?.?m  ?.?kg"
	# All text grey for undiscovered
	_preview_element.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
	_preview_name.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
	_preview_physical.add_theme_color_override("font_color", Color.BLACK)
	_preview_stats.add_theme_color_override("font_color", Color(0.4, 0.4, 0.4))
